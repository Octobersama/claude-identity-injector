...
func parseConfig(raw []byte) (config, error) {
	...
	for index := range next.Rules {
		...
		if !validStrictProfile(rule.StrictProfile) {
			return config{}, fmt.Errorf("rule %q strict_profile must be minimal, bearer, bearer_http1, identity, system, body, headers, body_headers, or full", rule.ID)
		}
...
func validStrictProfile(value string) bool {
	...
	case "minimal", "bearer", "bearer_http1", "identity", "system", "body", "headers", "body_headers", "full":
		return true
	default:
		return false
	}
